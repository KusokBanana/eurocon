...
(function ($) {
    ...
    var methods = {
        ...
        reload: function (args) {
            var type = (args !== undefined && args.type) ? args.type : 'reload';
            var $chat = $(this);
            methods.resetForm.apply($chat);
            methods.empty.apply($chat);
            methods.load.apply($chat, [{type: type}]);
        },
    ...
    var find = function ($chat, id) {
        ...
        var $container = $chat.find(widget.settings.container);
        if(typeof id == 'number'){
            return $container.find('[data-key=' + id + ']').parent('.msg');
        } else if(id == 'last') {
            return $container.find('.' + widget.settings.itemCssClass).last().find('.chat-content').last();
        } else if(id == 'first') {
            return $container.find('.' + widget.settings.itemCssClass).first().find('.chat-content').first();
        }else{
